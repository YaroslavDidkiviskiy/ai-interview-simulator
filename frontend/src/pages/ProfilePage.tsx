...
import { Target, Flame, Star, Globe, Rocket, Calendar, AlertTriangle, ChevronRight } from 'lucide-react'
import {
...
import { getMyStats, ProfileStats } from '../api/client'
        ...
        {days.map(d => (
          <div
            ...
            style={{
              ...
              background: d.count === 0 ? '#1e293b' : d.count === 1 ? '#4f46e570' : d.count === 2 ? '#4f46e5' : '#7c3aed',
...
function Skeleton({ h = 16, w = '100%' }: { h?: number; w?: string }) {
  return (
    <div style={{ height: h, width: w, background: '#1e293b', borderRadius: 6, animation: 'pulse 1.6s ease-in-out infinite' }} />
  )
}
  ...
  return (
    <Layout>
      ...
      <div className="fu" style={{ marginBottom: 32 }}>
        <h1 style={{ margin: '0 0 4px', fontSize: 26, fontWeight: 800, color: '#f1f5f9', letterSpacing: '-0.02em' }}>Profile</h1>
      ...
      <div className="fu" style={{ animationDelay: '.05s', display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 24 }}>
        {[
          ...
          <div key={item.label} style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 14, padding: '14px 16px', textAlign: 'center' }}>
      ...
      <div className="fu" style={{ animationDelay: '.1s', background: '#0f172a', border: '1px solid #1e293b', borderRadius: 16, padding: '20px 24px', marginBottom: 20 }}>
        {loading ? <Skeleton h={18} w="40%" /> : <ActivityGrid activity={stats?.activity ?? {}} />}
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
        <div className="fu" style={{ animationDelay: '.15s', background: '#0f172a', border: '1px solid #1e293b', borderRadius: 16, padding: '20px 24px' }}>
          <div style={{ fontSize: 11, color: '#475569', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 14 }}>By role</div>
          {loading ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>{[1,2,3].map(i => <Skeleton key={i} h={12} />)}</div>
                ...
                <div key={r.role} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                      <span style={{ fontSize: 12, color: '#94a3b8', fontWeight: 500 }}>{formatRoleLabel(r.role)}</span>
                      <span style={{ fontSize: 12, color: scoreColor(r.avg_score), fontWeight: 700 }}>{r.avg_score}</span>
                    </div>
                    <div style={{ height: 3, background: '#1e293b', borderRadius: 999 }}>
                      <div style={{ height: '100%', borderRadius: 999, width: `${(r.avg_score / 10) * 100}%`, background: scoreColor(r.avg_score), transition: 'width .6s cubic-bezier(.4,0,.2,1)' }} />
                    </div>
                  </div>
        ...
        <div className="fu" style={{ animationDelay: '.2s', background: '#0f172a', border: '1px solid #1e293b', borderRadius: 16, padding: '20px 24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14 }}>
            <AlertTriangle size={12} color="#f87171" />
            <span style={{ fontSize: 11, color: '#475569', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em' }}>Weak zones</span>
          </div>
          {loading ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>{[1,2,3].map(i => <Skeleton key={i} h={12} />)}</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {stats?.weak_topics.map(t => (
                <div key={t.topic} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                      <span style={{ fontSize: 12, color: '#94a3b8', fontWeight: 500 }}>{formatTopicLabel(t.topic)}</span>
                      <span style={{ fontSize: 12, color: scoreColor(t.avg_score), fontWeight: 700 }}>{t.avg_score}</span>
                    </div>
                    <div style={{ height: 3, background: '#1e293b', borderRadius: 999 }}>
                      <div style={{ height: '100%', borderRadius: 999, width: `${(t.avg_score / 10) * 100}%`, background: scoreColor(t.avg_score), transition: 'width .6s cubic-bezier(.4,0,.2,1)' }} />
                    </div>
                  </div>
      ...
      <div className="fu" style={{ animationDelay: '.25s', background: '#0f172a', border: '1px solid #1e293b', borderRadius: 16, padding: '20px 24px', marginBottom: 24 }}>
        <div style={{ fontSize: 11, color: '#475569', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 16 }}>Achievements</div>
        {loading ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
            {stats?.achievements.map(a => (
              <div key={a.id} style={{
                ...
                <div style={{ marginBottom: 8 }}><AchievementIcon id={a.id} unlocked={a.unlocked} /></div>
                <div style={{ fontSize: 12, fontWeight: 700, color: a.unlocked ? '#e2e8f0' : '#475569', marginBottom: 3 }}>{a.title}</div>
                <div style={{ fontSize: 11, color: '#475569', lineHeight: 1.4 }}>{a.desc}</div>
                {a.unlocked && <div style={{ marginTop: 6, fontSize: 10, color: '#6366f1', fontWeight: 600 }}>✓ Unlocked</div>}
              </div>
            ))}
          </div>
        )}
      ...
      <div className="fu" style={{ animationDelay: '.3s', textAlign: 'center' }}>
        <Link to="/sessions/create" style={{
          ...
          padding: '11px 24px', borderRadius: 12, fontWeight: 700, fontSize: 14, boxShadow: '0 4px 20px #4f46e530',
        }}>
