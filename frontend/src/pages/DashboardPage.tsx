    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 12, color: '#64748b' }}>{formatLevelLabel(session.level)}</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          {isCompleted && session.final_score != null && (
            <MiniRing value={session.final_score} />
          )}
          <span style={{
...
function FilterBar({ current, onChange }: { current: FilterStatus; onChange: (f: FilterStatus) => void }) {
  const opts: { value: FilterStatus; label: string }[] = [
    { value: 'all',       label: 'All' },
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/users/me/sessions?page=${page}&limit=${LIMIT}`, {
