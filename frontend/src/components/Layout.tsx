...
import { LogOut, User, LayoutDashboard, Settings, ChevronDown, Mail, ShieldCheck } from 'lucide-react'
import { useAuth } from '../context/AuthContext'
import EmailVerificationModal, { getSavedCooldown, COOLDOWN_SECS, COOLDOWN_KEY } from './EmailVerificationModal'
...
export default function Layout({ children, variant = 'default' }: Props) {
  ...
  const [codeSentBefore, setCodeSentBefore] = useState(false)

  useEffect(() => {
    const { cooldown, codeSent } = getSavedCooldown()
    setBannerCooldown(cooldown)
    setCodeSentBefore(codeSent)
  }, [])

  useEffect(() => {
    if (bannerCooldown <= 0) return
    const t = setTimeout(() => setBannerCooldown(c => c - 1), 1000)
  ...
  function handleCodeSent(ts: number) {
    localStorage.setItem(COOLDOWN_KEY, String(ts))
    setBannerCooldown(COOLDOWN_SECS)
    setCodeSentBefore(true)
  }

  const showBanner = !isLoading && user?.auth_provider === 'local' && !user?.email_verified

  const { cooldown: modalInitCooldown, codeSent: modalInitCodeSent } = showModal
    ? getSavedCooldown()
    : { cooldown: 0, codeSent: false }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-950">
      <style>{`
        ...
        .verify-banner-btn:hover { background: #78350f !important; border-color: #b45309 !important; }
      `}</style>
            ...
            {user ? (
              <div className="flex items-center gap-2">
                <div className="relative" ref={dropdownRef}>
                  ...
                  >
                    <span className="hidden sm:block truncate max-w-[180px] text-xs" style={{ color: '#94a3b8' }}>
                      {user.email}
                    </span>
                    <ChevronDown style={{
                      width: 13, height: 13,
                  ...
                  {open && (
                    <div style={{
                      ...
                      padding: '6px', boxShadow: '0 16px 48px #00000060',
                      animation: 'dropIn .15s cubic-bezier(.4,0,.2,1)', zIndex: 50,
                    }}>
                      <div style={{ padding: '8px 10px 10px', borderBottom: '1px solid #1e293b', marginBottom: 4 }}>
                      ...
                      <div style={{ borderTop: '1px solid #1e293b', marginTop: 4, paddingTop: 4 }}>
                        <button
                          ...
                          onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = '#f8717115' }}
      ...
      {showBanner && (
        <div style={{
          background: 'linear-gradient(90deg, #422006ee, #431407ee)',
          borderBottom: '1px solid #92400e60',
          animation: 'bannerSlideDown .2s ease both',
        }}>
          <div style={{
            maxWidth: 896, margin: '0 auto', padding: '8px 24px',
            display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            gap: 12, flexWrap: 'wrap',
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <Mail size={14} style={{ color: '#fbbf24', flexShrink: 0 }} />
            ...
            >
              {bannerCooldown > 0
                ? <>{`Resend in ${bannerCooldown}s`}</>
                : <><ShieldCheck size={12} />{codeSentBefore ? 'Enter code' : 'Verify email'}</>}
            </button>
          </div>
        </div>
      )}
      ...
      {showModal && (
        <EmailVerificationModal
          ...
          initialCooldown={modalInitCooldown}
          initialCodeSent={modalInitCodeSent}
